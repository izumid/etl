...
def main():
	try:
		...
		if len(config["BASEFILE"]["destination"]) > 3:
			...
			postgres.get_data(connection=conn, sql_query=f"query/{config["QUERY"]["name_file"]}",abs_path_destination=abs_path_destination,data_feather=config["BASEFILE"]["name"],debug=debug)
